...
## Types  
          ...
          <DocBlock title="LRStateItem" kind="type">

```mc
type LRStateItem : { nt: Name, terms: [Term], stackPointer: Int, lookahead: [TokenRepr], prodIdx: Int }
```
...
```
          ...
          <DocBlock title="LRState" kind="type">
...
```
          ...
          <DocBlock title="LRParseTable" kind="type">

```mc
type LRParseTable : { k_lookahead: Int, entrypointIdx: Int, entrypointProdIdx: Int, syntaxDef: SyntaxDef, nonTerminalTypes: Map Name Type, tokenConTypes: Map TokenRepr { conIdent: Name, conArg: Type }, states: [Set LRStateItem], shifts: Map Int [{ lookahead: [TokenRepr], toIdx: Int }], gotos: Map Int [{ nt: Name, toIdx: Int }], reductions: Map Int [{ lookahead: [TokenRepr], prodIdx: Int }] }
```
...
```
          ...
          <DocBlock title="LRGeneratorBindings" kind="type">

```mc
type LRGeneratorBindings : { t_Result: Name, t_Info: Name, t_Token: Name, t_Stream: Name, c_NoInfo: Name, c_ResultOk: Name, c_ResultErr: Name, v_result_ok: Expr, v_result_err: Expr, v_nextToken: Expr, v_stream: Expr, v_mergeInfo: Expr, v_tokInfo: Expr, v_strJoin: Expr, v_tokToStr: Expr, v_join: Expr, v_int2string: Expr }
```

<Description>{`Generates an expression for a LR\(k\) parser that begins parsing as soon as  
...
Example usage:  
   ...
   let bindings = \{lrDefaultGeneratorBindings \(\) with  
     v\_stream = nvar\_ nStream,  
     v\_nextToken = nvar\_ nNextToken  
   \} in  
   let parserExpr = nulams\_ \[nStream, nNextToken\] \(  
...
The generated LR\(k\) parser expression will follow the following stucture:  
   ...
   let gotoLookup\_\<NT0\> = \[...\] in  
   let gotoLookup\_\<NT1\> = \[...\] in  
   ...  
   let initialStacks = \{  
     typeStack0 = toList \[\],  
     typeStack1 = toList \[\],  
   ...
   let lookahead\_check: Result w e \(Stream, \[Token\]\) =  
     ...  
   in  
   switch lookahead\_check  
           ...
           case 0 then  
             ...
             case \[TokenA \_, ...\] then  
               ...
               \<otherwise \(majority of cases\)\>  
                 ...
                 let nextState = get \(head stateTrace\) gotoLookup\_\<reduced NT\> in  
...
## Semantics  
          ...
          <DocBlock title="lrStateItemCmp2" kind="sem">

```mc
sem lrStateItemCmp2 : (LRParser_LRStateItem, LRParser_LRStateItem) -> Int
```
...
```
          ...
          <DocBlock title="lrStateItemCmp" kind="sem">

```mc
sem lrStateItemCmp : LRParser_LRStateItem -> LRParser_LRStateItem -> Int
```
...
```
          ...
          <DocBlock title="lrClosure" kind="sem">

```mc
sem lrClosure : Int -> ContextFreeGrammar_SyntaxDef -> Map ContextFreeGrammar_Term (Set [TokenReprBase_TokenRepr]) -> Set LRParser_LRStateItem -> Set LRParser_LRStateItem
```

<Description>{`Computes the closure for the input set as  
Closure\(I\) =  
  repeat  
    for any item \(A \-\> a.Xb, L\) in I  
      for any production X \-\> y  
        for any W in FIRST\_k \(bL\)  
          I \<\- I U \{\(X \-\> .y, W\)\}  
  until I does not change  
  return INo documentation available here.`}</Description>
...
```
          ...
          <DocBlock title="lrGoto" kind="sem">

```mc
sem lrGoto : Int -> ContextFreeGrammar_SyntaxDef -> Map ContextFreeGrammar_Term (Set [TokenReprBase_TokenRepr]) -> Set LRParser_LRStateItem -> ContextFreeGrammar_Term -> Set LRParser_LRStateItem
```

<Description>{`GOTO\(I, X\) =  
  J \<\- \{\}  
  for any item \(A \-\> a.Xb, L\) in I  
    add \(A \-\> aX.b, L\) to J  
  return Closure\(J\)No documentation available here.`}</Description>
...
```
          ...
          <DocBlock title="lrCreateParseTable" kind="sem">

```mc
sem lrCreateParseTable : Int -> Map TokenReprBase_TokenRepr {conArg: Ast_Type, conIdent: Name} -> ContextFreeGrammar_SyntaxDef -> Result String String LRParser_LRParseTable
```

<Description>{`\-\- Generate the LR\(k\) parse table.
 \-\-
 \-\- Arguments:
 \-\-  \- k: Int
 \-\-      Lookahead for the parser. Must be greater than or equal to 1.
...
```
          ...
          <DocBlock title="lrtable2string" kind="sem">

```mc
sem lrtable2string : Int -> LRParser_LRParseTable -> String
```

<Description>{`Creates a multi\-line representation of the LR parse table, with each line  
indented by the specified amount of whitespaces.No documentation available here.`}</Description>
...
```
          ...
          <DocBlock title="lrDefaultGeneratorBindings" kind="sem">

```mc
sem lrDefaultGeneratorBindings : () -> LRParser_LRGeneratorBindings
```
...
```
          ...
          <DocBlock title="lrGenerateParser" kind="sem">

```mc
sem lrGenerateParser : LRParser_LRGeneratorBindings -> LRParser_LRParseTable -> Ast_Expr
```
