...
<a href={"/docs/Stdlib/dfa.mc"} style={S.link}>dfa.mc</a>, <a href={"/docs/Stdlib/eqset.mc"} style={S.link}>eqset.mc</a>  
  
## Types  
  

          <DocBlock title="RegEx" kind="type">

```mc
type RegEx
```

<Description>{`Represents basic regular expressions.`}</Description>
...
```
          ...
          <DocBlock title="Epsilon" kind="con">
...
```
          ...
          <DocBlock title="Symbol" kind="con">
...
```
          ...
          <DocBlock title="Concat" kind="con">
...
```
          ...
          <DocBlock title="Kleene" kind="con">
...
## Variables  
          ...
          <DocBlock title="prec" kind="let">

```mc
let prec reg : all a. RegEx a -> Int
```
...
```
          ...
          <DocBlock title="regEx2str" kind="let">

```mc
let regEx2str sym2str reg : all a. (a -> String) -> RegEx a -> String
```
...
```
          ...
          <DocBlock title="regExPprint" kind="let">

```mc
let regExPprint sym2str reg : all a. (a -> String) -> RegEx a -> ()
```
...
```
          ...
          <DocBlock title="eqr" kind="let">

```mc
let eqr eqsym re1 re2 : all a. all a1. (a -> a1 -> Bool) -> RegEx a -> RegEx a1 -> Bool
```

<Description>{`Checks structural equivalence of two regexes.  
Does NOT check whether the regexes describe the same language.`}</Description>
...
```
          ...
          <DocBlock title="simplifyS" kind="let">

```mc
let simplifyS reg : all a. RegEx a -> RegEx a
```
...
```
          ...
          <DocBlock title="regexFromDFA" kind="let">

```mc
let regexFromDFA dfa : all a. all a1. NFA a1 a -> RegEx a
```

<Description>{`Convert a DFA into a regular expression using State Elimination Method.  
From the book: John E. Hopcroft, Jeffrey D. Ullman \(1979\). Introduction  
to Automata Theory, Languages, and Computation.`}</Description>
