...
<a href={"/docs/Stdlib/option.mc"} style={S.link}>option.mc</a>, <a href={"/docs/Stdlib/seq.mc"} style={S.link}>seq.mc</a>, <a href={"/docs/Stdlib/string.mc"} style={S.link}>string.mc</a>  
  
## Types  
  

          <DocBlock title="TCreate" kind="type">
...
```
          ...
          <DocBlock title="linearToCartesianIndex" kind="let">

```mc
let linearToCartesianIndex shape k : [Int] -> Int -> [Int]
```

<Description>{`Converts linear index \`k\` to Cartesian index in row\-major order.`}</Description>
...
```
          ...
          <DocBlock title="cartesianToLinearIndex" kind="let">

```mc
let cartesianToLinearIndex shape idx : [Int] -> [Int] -> Int
```

<Description>{`Converts Cartesian index \`idx\` to linear index assuming row\-major order.`}</Description>
...
```
          ...
          <DocBlock title="indexFoldu" kind="let">

```mc
let indexFoldu f acc start stop : all a. (a -> Int -> a) -> a -> Int -> Int -> a
```

<Description>{`Folds \`f\` over the range \`start\` \`stop\` using accumulator \`acc\``}</Description>
...
```
          ...
          <DocBlock title="indexFoldRM" kind="let">

```mc
let indexFoldRM f acc shape : all a. (a -> [Int] -> a) -> a -> [Int] -> a
```

<Description>{`Folds \`f\` over the indexes up to \`shape\` in row\-major order and accumulator  
\`acc\``}</Description>
...
```
          ...
          <DocBlock title="optionIndexFoldRMM" kind="let">

```mc
let optionIndexFoldRMM f acc shape : all a. (a -> [Int] -> Option a) -> a -> [Int] -> Option a
```

<Description>{`Folds \`f\` over the indexes of \`shape\` in row\-major order with accumulator  
\`acc\`. If \`f acc idx\` is \`None \(\)\` then the result is \`None \(\)\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorHasRank" kind="let">

```mc
let tensorHasRank t rank : all a. Tensor[a] -> Int -> Bool
```
...
```
          ...
          <DocBlock title="tensorHasShape" kind="let">

```mc
let tensorHasShape t shape : all a. Tensor[a] -> [Int] -> Bool
```
...
```
          ...
          <DocBlock title="tensorHasSameShape" kind="let">

```mc
let tensorHasSameShape t1 t2 : all a. all b. Tensor[a] -> Tensor[b] -> Bool
```
...
```
          ...
          <DocBlock title="tensorCreate" kind="let">

```mc
let tensorCreate  : all a. [Int] -> ([Int] -> a) -> Tensor[a]
```

<Description>{`Default to dense tensors`}</Description>
...
```
          ...
          <DocBlock title="tensorOfSeqOrElse" kind="let">

```mc
let tensorOfSeqOrElse f tcreate shape seq : all a. (() -> Tensor[a]) -> ([Int] -> ([Int] -> a) -> Tensor[a]) -> [Int] -> [a] -> Tensor[a]
```

<Description>{`Construct a tensor of shape \`shape\` from a sequence \`seq\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorOfSeqExn" kind="let">

```mc
let tensorOfSeqExn x : all a. ([Int] -> ([Int] -> a) -> Tensor[a]) -> [Int] -> [a] -> Tensor[a]
```
...
```
          ...
          <DocBlock title="tensorToSeqOrElse" kind="let">

```mc
let tensorToSeqOrElse f t : all a. (() -> [a]) -> Tensor[a] -> [a]
```

<Description>{`Construct a sequence from a rank 1 tensor \`t\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorToSeqExn" kind="let">

```mc
let tensorToSeqExn x : all a. Tensor[a] -> [a]
```
...
```
          ...
          <DocBlock title="tensorDenseRepeat" kind="let">

```mc
let tensorDenseRepeat shape v : all a. [Int] -> a -> Tensor[a]
```

<Description>{`Create a tensor filled with values \`v\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorSize" kind="let">

```mc
let tensorSize t : all a. Tensor[a] -> Int
```

<Description>{`The number of elements in a tensor \`t\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorMapOrElse" kind="let">

```mc
let tensorMapOrElse f g t1 t2 : all a. all b. (() -> ()) -> (a -> b -> b) -> Tensor[a] -> Tensor[b] -> ()
```

<Description>{`Map the elements of \`t1\` to the elements of \`t2\` via the function \`f\`,  
where \`t1\` and \`t2\` has to have the same shape.`}</Description>
...
```
          ...
          <DocBlock title="tensorMapExn" kind="let">

```mc
let tensorMapExn x : all a. all b. (a -> b -> b) -> Tensor[a] -> Tensor[b] -> ()
```
...
```
          ...
          <DocBlock title="tensorMapInplace" kind="let">

```mc
let tensorMapInplace f t : all a. (a -> a) -> Tensor[a] -> ()
```

<Description>{`Applies function \`f\` to the elements of \`t\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorMapCopy" kind="let">

```mc
let tensorMapCopy f t : all a. (a -> a) -> Tensor[a] -> Tensor[a]
```

<Description>{`Applies function \`f\` to the elements of a copy of \`t\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorMapiOrElse" kind="let">

```mc
let tensorMapiOrElse f g t1 t2 : all a. all b. (() -> ()) -> ([Int] -> a -> b -> b) -> Tensor[a] -> Tensor[b] -> ()
```

<Description>{`Map the index and elements of \`t1\` to the elements of \`t2\` via the function  
\`f\`, where \`t1\` and \`t2\` has to have the same shape.`}</Description>
...
```
          ...
          <DocBlock title="tensorMapiExn" kind="let">

```mc
let tensorMapiExn x : all a. all b. ([Int] -> a -> b -> b) -> Tensor[a] -> Tensor[b] -> ()
```
...
```
          ...
          <DocBlock title="tensorMapiInplace" kind="let">

```mc
let tensorMapiInplace f t : all a. ([Int] -> a -> a) -> Tensor[a] -> ()
```
...
```
          ...
          <DocBlock title="tensorMapiCopy" kind="let">

```mc
let tensorMapiCopy f t : all a. ([Int] -> a -> a) -> Tensor[a] -> Tensor[a]
```
...
```
          ...
          <DocBlock title="tensorBlitExn" kind="let">

```mc
let tensorBlitExn t1 t2 : all a. Tensor[a] -> Tensor[a] -> ()
```

<Description>{`Copies the content of \`t1\` to \`t2\`. Gives an error if \`t1\` and \`t2\` does no  
have the same shape.`}</Description>
...
```
          ...
          <DocBlock title="tensorFoldliSlice" kind="let">

```mc
let tensorFoldliSlice f acc t1 : all a. all b. (b -> Int -> Tensor[a] -> b) -> b -> Tensor[a] -> b
```

<Description>{`Left folds \`f acc idx t\` over the zero'th dimension of \`t1\`, where \`acc\` is  
...
```
          ...
          <DocBlock title="tensorFoldlSlice" kind="let">

```mc
let tensorFoldlSlice f : all a. all b. (b -> Tensor[a] -> b) -> b -> Tensor[a] -> b
```

<Description>{`Left folds \`f acc t\` over the zero'th dimension of \`t1\`, where \`acc\` is the  
accumulator and \`t\` is the i'th slice of \`t1\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorFold" kind="let">

```mc
let tensorFold f acc t : all a. all b. (b -> a -> b) -> b -> Tensor[a] -> b
```

<Description>{`Folds \`f acc el\` over all elements \`el\` of \`t\` in row\-major order, where  
\`acc\` is the accumulator.`}</Description>
...
```
          ...
          <DocBlock title="tensorFoldi" kind="let">

```mc
let tensorFoldi f acc t : all a. all b. (b -> [Int] -> a -> b) -> b -> Tensor[a] -> b
```

<Description>{`Folds \`f idx acc el\` over all elements \`el\` of \`t\` in row\-major order, where  
\`acc\` is the accumulator and \`idx\` is the index of the element.`}</Description>
...
```
          ...
          <DocBlock title="tensorIteri" kind="let">

```mc
let tensorIteri f t : all a. ([Int] -> a -> ()) -> Tensor[a] -> ()
```

<Description>{`Iterates through the elements of \`t\` in row\-major order, applying the  
function \`f\` on each index and element.`}</Description>
...
```
          ...
          <DocBlock title="tensorIter" kind="let">

```mc
let tensorIter f : all a. (a -> ()) -> Tensor[a] -> ()
```

<Description>{`Iterates through the elements of \`t\` in row\-major order, applying the  
function \`f\` on each element.`}</Description>
...
```
          ...
          <DocBlock title="tensorMax" kind="let">

```mc
let tensorMax cmp t : all a. (a -> a -> Int) -> Tensor[a] -> Option a
```

<Description>{`The maximum element in \`t\` as defined by \`cmp\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorMin" kind="let">

```mc
let tensorMin cmp t : all a. (a -> a -> Int) -> Tensor[a] -> Option a
```

<Description>{`The minimum element in \`t\` as defined by \`cmp\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorFindi" kind="let">

```mc
let tensorFindi p t : all a. (a -> Bool) -> Tensor[a] -> Option (a, [Int])
```

<Description>{`Finds element and index \`Some \(el, i\)\` in \`t\` satisfying predicate \`p\`. If  
no such element is found then \`None\` is returned.`}</Description>
...
```
          ...
          <DocBlock title="tensorFind" kind="let">

```mc
let tensorFind p t : all a. (a -> Bool) -> Tensor[a] -> Option a
```

<Description>{`Finds element \`Some el\` in \`t\` satisfying predicate \`p\`. If  
no such element is found then \`None\` is returned.`}</Description>
...
```
          ...
          <DocBlock title="tensorIndex" kind="let">

```mc
let tensorIndex p t : all a. (a -> Bool) -> Tensor[a] -> Option [Int]
```

<Description>{`Finds index \`Some i\` in \`t\` of element satisfying predicate \`p\`. If no such  
element is found then \`None\` is returned.`}</Description>
...
```
          ...
          <DocBlock title="tensorAny" kind="let">

```mc
let tensorAny p t : all a. (a -> Bool) -> Tensor[a] -> Bool
```

<Description>{`\`true\` if \`p x\` for some \`x\` in \`t\`, else \`false\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorAll" kind="let">

```mc
let tensorAll p t : all a. (a -> Bool) -> Tensor[a] -> Bool
```

<Description>{`\`true\` if \`p x\` for all \`x\` in \`t\`, else \`false\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorDimsEqual" kind="let">

```mc
let tensorDimsEqual t : all a. Tensor[a] -> Bool
```

<Description>{`All dimensions of tensor \`t\` are of the same size.`}</Description>
...
```
          ...
          <DocBlock title="tensorFilter" kind="let">

```mc
let tensorFilter p t : all a. (a -> Bool) -> Tensor[a] -> [a]
```

<Description>{`Filter elements of \`t\` given predicate \`p\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorFilteri" kind="let">

```mc
let tensorFilteri p t : all a. ([Int] -> a -> Bool) -> Tensor[a] -> [[Int]]
```

<Description>{`Filter index of elements of \`t\` given predicate \`p\`.`}</Description>
...
```
          ...
          <DocBlock title="tensorSubSeqExn" kind="let">

```mc
let tensorSubSeqExn tcreate t sub : all a. TCreate a -> Tensor[a] -> [[Int]] -> Tensor[a]
```

<Description>{`Given a tensor and sequence of index sequences, produce the corresponding  
...
tests for example usages.`}</Description>
...
```
          ...
          <DocBlock title="tensorCumsumiExn" kind="let">

```mc
let tensorCumsumiExn t r : Tensor[Int] -> Tensor[Int] -> ()
```
...
```
          ...
          <DocBlock title="tensorCumsumiInplace" kind="let">

```mc
let tensorCumsumiInplace t : Tensor[Int] -> ()
```
...
```
          ...
          <DocBlock title="tensorCumsumiCopy" kind="let">

```mc
let tensorCumsumiCopy t : Tensor[Int] -> Tensor[Int]
```
