...
<a href={"/docs/Stdlib/multicore/thread-pool.mc"} style={S.link}>thread-pool.mc</a>, <a href={"/docs/Stdlib/seq.mc"} style={S.link}>seq.mc</a>  
  
## Variables  
  

          <DocBlock title="_joinReverseList" kind="let">

```mc
let _joinReverseList seqs : all a. [[a]] -> [a]
```

<Description>{`Reverse join for lists \(tail recursive\)`}</Description>
...
```
          ...
          <DocBlock title="_split" kind="let">

```mc
let _split seq chunkSize : all a. [a] -> Int -> [[a]]
```

<Description>{`Split a sequence into chunks of size 'chunkSize'`}</Description>
...
```
          ...
          <DocBlock title="pmap" kind="let">

```mc
let pmap pool nbrChunks f seq : all a. all b. ThreadPool [b] -> Int -> (a -> b) -> [a] -> [b]
```

<Description>{`'pmap pool nbrChunks f s' applies 'f' to all elements in 's' in parallel,  
...
as output, and vice versa for Ropes.`}</Description>
