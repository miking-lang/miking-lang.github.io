...
<a href={"/docs/Stdlib/option.mc"} style={S.link}>option.mc</a>, <a href={"/docs/Stdlib/bool.mc"} style={S.link}>bool.mc</a>  
  
## Variables  
  

          <DocBlock title="make" kind="let">

```mc
let make n v : all a. Int -> a -> [a]
```
...
```
          ...
          <DocBlock title="eqSeq" kind="let">

```mc
let eqSeq eq s1 s2 : all a. all b. (a -> b -> Bool) -> [a] -> [b] -> Bool
```
...
```
          ...
          <DocBlock title="toRope" kind="let">

```mc
let toRope seq : all a. [a] -> [a]
```

<Description>{`Converting between List and Rope`}</Description>
...
```
          ...
          <DocBlock title="toList" kind="let">

```mc
let toList seq : all a. [a] -> [a]
```
...
```
          ...
          <DocBlock title="mapOption" kind="let">

```mc
let mapOption f : all a. all b. (a -> Option b) -> [a] -> [b]
```
...
```
          ...
          <DocBlock title="mapiOption" kind="let">

```mc
let mapiOption f : all a. all b. (Int -> a -> Option b) -> [a] -> [b]
```
...
```
          ...
          <DocBlock title="for_" kind="let">

```mc
let for_ xs f : all a. [a] -> (a -> ()) -> ()
```
...
```
          ...
          <DocBlock title="mapReverse" kind="let">

```mc
let mapReverse f lst : all a. all b. (a -> b) -> [a] -> [b]
```

<Description>{`In contrast to map, mapReverse is tail recursive.`}</Description>
...
```mc
let foldl1 f l : all a. (a -> a -> a) -> [a] -> a
```

<Description>{`Folds`}</Description>
...
```mc
let foldr1 f seq : all a. (a -> a -> a) -> [a] -> a
```
...
```
          ...
          <DocBlock title="range" kind="let">

```mc
let range s e by : Int -> Int -> Int -> [Int]
```
...
```
          ...
          <DocBlock title="foldli" kind="let">

```mc
let foldli fn initAcc seq : all a. all b. (a -> Int -> b -> a) -> a -> [b] -> a
```

<Description>{`\`foldli f acc seq\` folds over a sequence together with the index of element  
in the sequence. \(Similar to \`mapi\`\)`}</Description>
...
```
          ...
          <DocBlock title="mapK" kind="let">

```mc
let mapK f seq k : all a. all b. all c. (a -> (b -> c) -> c) -> [a] -> ([b] -> c) -> c
```

<Description>{`\`mapK f seq k\` maps the continuation passing function \`f\` over the sequence  
\`seq\`, passing the result of the mapping to the continuation \`k\`.`}</Description>
...
```
          ...
          <DocBlock title="mapiK" kind="let">

```mc
let mapiK f seq k : all a. all b. all c. (Int -> a -> (b -> c) -> c) -> [a] -> ([b] -> c) -> c
```

<Description>{`\`mapiK f seq k\` maps the continuation passing function \`f\` with the index  
over the sequence \`seq\`, passing the result of the mapping to the  
continuation \`k\`.`}</Description>
...
```
          ...
          <DocBlock title="foldlK" kind="let">

```mc
let foldlK f acc seq k : all a. all b. all c. all d. (a -> b -> (a -> c) -> c) -> a -> [b] -> (a -> c) -> c
```

<Description>{`\`foldlK f acc seq k\` fold the continuation passing function \`f\` over the  
sequence \`seq\`, from the left, with the initial accumulator \`acc\` and  
continuation \`k\`. \(from  
https://leastfixedpoint.com/tonyg/kcbbs/lshift\_archive/folds\-and\-continuation\-passing\-style\-20070611.html\)`}</Description>
...
```
          ...
          <DocBlock title="iterK" kind="let">

```mc
let iterK f seq k : all a. all b. (a -> (() -> ()) -> ()) -> [a] -> (() -> ()) -> ()
```

<Description>{`\`iterK f seq k\` iteratively applies the function \`f\` to the elements of \`seq\`  
as long as \`\(\)\` is passed to its continuation.`}</Description>
...
```
          ...
          <DocBlock title="zipWith" kind="let">

```mc
let zipWith f : all a. all b. all c. (a -> b -> c) -> [a] -> [b] -> [c]
```
...
```
          ...
          <DocBlock title="zipWithIndex" kind="let">

```mc
let zipWithIndex f a1 a2 : all a. all b. all c. (Int -> a -> b -> c) -> [a] -> [b] -> [c]
```
...
```
          ...
          <DocBlock title="zip" kind="let">

```mc
let zip l1 l2 : all a. all b. [a] -> [b] -> [(a, b)]
```
...
```mc
let mapAccumL f acc seq : all a. all b. all c. (a -> b -> (a, c)) -> a -> [b] -> (a, [c])
```

<Description>{`Accumulating maps`}</Description>
...
```mc
let mapAccumR f acc seq : all a. all b. all c. (a -> b -> (a, c)) -> a -> [b] -> (a, [c])
```
...
```
          ...
          <DocBlock title="unzip" kind="let">

```mc
let unzip l : all a. all b. [(a, b)] -> ([a], [b])
```
...
```mc
let iter2 f seq1 seq2 : all a. all b. (a -> b -> ()) -> [a] -> [b] -> ()
```

<Description>{`\`iter2 f seq1 seq1\` iterativly applies \`f\` to the first  
min\(\`length seq1\`, \`length seq2\`\) elements in \`seq1\` and \`seq2\`.`}</Description>
...
```
          ...
          <DocBlock title="joinMap" kind="let">

```mc
let joinMap f a : all a. all b. (a -> [b]) -> [a] -> [b]
```
...
```
          ...
          <DocBlock title="seqLiftA2" kind="let">

```mc
let seqLiftA2 f as bs : all a. all b. all c. (a -> b -> c) -> [a] -> [b] -> [c]
```
...
```
          ...
          <DocBlock title="seqLiftA3" kind="let">

```mc
let seqLiftA3 f as bs cs : all a. all b. all c. all d. (a -> b -> c -> d) -> [a] -> [b] -> [c] -> [d]
```
...
```
          ...
          <DocBlock title="seqMapM" kind="let">

```mc
let seqMapM f : all a. all b. (a -> [b]) -> [a] -> [[b]]
```
...
```
          ...
          <DocBlock title="findLast" kind="let">

```mc
let findLast p seq : all a. (a -> Bool) -> [a] -> Option a
```

<Description>{`Find the last element in a sequence satisfying a predicate in a list.`}</Description>
...
```
          ...
          <DocBlock title="findi" kind="let">

```mc
let findi p seq : all a. (a -> Bool) -> [a] -> Option Int
```

<Description>{`Find the first index in a sequence whose element satisfies a predicate`}</Description>
...
```
          ...
          <DocBlock title="findiLast" kind="let">

```mc
let findiLast p seq : all a. (a -> Bool) -> [a] -> Option Int
```

<Description>{`Find the last index in a sequence whose element satisfies a predicate`}</Description>
...
```
          ...
          <DocBlock title="seqMem" kind="let">

```mc
let seqMem eq xs x : all a. (a -> a -> Bool) -> [a] -> a -> Bool
```
...
```
          ...
          <DocBlock title="lowerBoundBinarySearch" kind="let">

```mc
let lowerBoundBinarySearch f s : all a. (a -> Int) -> [a] -> Option Int
```

<Description>{`NOTE\(larshum, 2023\-05\-02\): Finds the minimum index in the given sequence for  
...
```mc
let partition p seq : all a. (a -> Bool) -> [a] -> ([a], [a])
```
...
```mc
let distinct eq seq : all a. (a -> a -> Bool) -> [a] -> [a]
```

<Description>{`Removes duplicates with preserved ordering. Keeps first occurrence of an element.`}</Description>
...
```
          ...
          <DocBlock title="distinctSorted" kind="let">

```mc
let distinctSorted eq s : all a. (a -> a -> Bool) -> [a] -> [a]
```

<Description>{`Removes duplicated elements in a sorted sequence. More efficient than the  
'distinct' function.`}</Description>
...
```
          ...
          <DocBlock title="minIdx" kind="let">

```mc
let minIdx cmp seq : all a. (a -> a -> Int) -> [a] -> Option (Int, a)
```

<Description>{`Max/Min`}</Description>
...
```
          ...
          <DocBlock title="min" kind="let">

```mc
let min cmp seq : all a. (a -> a -> Int) -> [a] -> Option a
```
...
```
          ...
          <DocBlock title="max" kind="let">

```mc
let max cmp : all a. (a -> a -> Int) -> [a] -> Option a
```
...
```
          ...
          <DocBlock title="minOrElse" kind="let">

```mc
let minOrElse d cmp seq : all a. (() -> a) -> (a -> a -> Int) -> [a] -> a
```
...
```
          ...
          <DocBlock title="maxOrElse" kind="let">

```mc
let maxOrElse d cmp : all a. (() -> a) -> (a -> a -> Int) -> [a] -> a
```
...
```
          ...
          <DocBlock title="index" kind="let">

```mc
let index pred seq : all a. (a -> Bool) -> [a] -> Option Int
```

<Description>{`First index in seq that satifies pred`}</Description>
...
```
          ...
          <DocBlock title="lastIndex" kind="let">

```mc
let lastIndex pred seq : all a. (a -> Bool) -> [a] -> Option Int
```

<Description>{`Last index in seq that satisfies pred`}</Description>
...
```
          ...
          <DocBlock title="indices" kind="let">

```mc
let indices pred seq : all a. (a -> Bool) -> [a] -> [Int]
```

<Description>{`Return a sequence of all indices for which the corresponding element  
satisfies the predicate.`}</Description>
...
```mc
let isSuffix eq s1 s2 : all a. all b. (a -> b -> Bool) -> [a] -> [b] -> Bool
```

<Description>{`Check if s1 is a suffix of s2`}</Description>
...
```
          ...
          <DocBlock title="seqCmp" kind="let">

```mc
let seqCmp cmp s1 s2 : all a. (a -> a -> Int) -> [a] -> [a] -> Int
```
...
```
          ...
          <DocBlock title="randIndex" kind="let">

```mc
let randIndex seq : all a. [a] -> Option Int
```

<Description>{`Select an index uniformly at random.`}</Description>
...
```
          ...
          <DocBlock title="randElem" kind="let">

```mc
let randElem seq : all a. [a] -> Option a
```

<Description>{`Select an element uniformly at random.`}</Description>
...
```
          ...
          <DocBlock title="permute" kind="let">

```mc
let permute elems permutation : all a. [a] -> [Int] -> [a]
```

<Description>{`Permute the order of elements according to a sequence of integers, which is  
...
```mc
let unfoldr f b0 : all a. all c. (a -> Option (c, a)) -> a -> [c]
```
...
```
          ...
          <DocBlock title="subseqReplace" kind="let">

```mc
let subseqReplace eq check replacement seq : all a. (a -> a -> Bool) -> [a] -> [a] -> [a] -> [a]
```

<Description>{`Replace all occurrences of the provided sequence \`check\` by another sequence  
\`replacement\`, in order of left to right.`}</Description>
