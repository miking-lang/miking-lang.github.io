...
## Semantics  
          ...
          <DocBlock title="generateCudaKernelFunctionNoRet" kind="sem">

```mc
sem generateCudaKernelFunctionNoRet : CExprTypeAst_CExpr -> (Name, CudaAst_CuTop)
```
...
```
          ...
          <DocBlock title="generateCudaKernelCallNoRet" kind="sem">

```mc
sem generateCudaKernelCallNoRet : MExprCCompileBase_CompileCEnv -> CExprTypeAst_CExpr -> (CudaAst_CuTop, CStmtAst_CStmt)
```
