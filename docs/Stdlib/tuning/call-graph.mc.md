...
<a href={"/docs/Stdlib/mexpr/ast.mc"} style={S.link}>mexpr/ast.mc</a>, <a href={"/docs/Stdlib/mexpr/symbolize.mc"} style={S.link}>mexpr/symbolize.mc</a>, <a href={"/docs/Stdlib/mexpr/anf.mc"} style={S.link}>mexpr/anf.mc</a>, <a href={"/docs/Stdlib/mexpr/ast-builder.mc"} style={S.link}>mexpr/ast-builder.mc</a>, <a href={"/docs/Stdlib/digraph.mc"} style={S.link}>digraph.mc</a>, <a href={"/docs/Stdlib/tuning/name-info.mc"} style={S.link}>name-info.mc</a>  
  
## Types  
  

          <DocBlock title="CallGraph" kind="type">
...
## Languages  
          ...
          <DocBlock title="HoleCallGraph" kind="lang" link="/docs/Stdlib/tuning/call-graph.mc/lang-HoleCallGraph">

```mc
lang HoleCallGraph
```

<Description>{`Construct a call graph from an AST. The nodes are named functions, and the  
...
is the set of nodes in the call graph \(i.e. set of functions in the AST\).`}</Description>
...
```
          ...
          <DocBlock title="TestLang" kind="lang" link="/docs/Stdlib/tuning/call-graph.mc/lang-TestLang">
...
## Variables  
          ...
          <DocBlock title="callGraphNames" kind="let">

```mc
let callGraphNames cg : all l. Digraph NameInfo l -> [Name]
```
...
```
          ...
          <DocBlock title="_callGraphNameSeq" kind="let">

```mc
let _callGraphNameSeq seq : [(NameInfo, NameInfo, NameInfo)] -> [(Name, Name, Name)]
```
...
```
          ...
          <DocBlock title="callGraphEdgeNames" kind="let">

```mc
let callGraphEdgeNames cg : Digraph NameInfo NameInfo -> [(Name, Name, Name)]
```
...
```
          ...
          <DocBlock title="callGraphTop" kind="let">

```mc
let callGraphTop  : (Name, Info)
```

<Description>{`The top of the call graph, has no incoming edges.`}</Description>
...
```
          ...
          <DocBlock title="_handleLetVertex" kind="let">

```mc
let _handleLetVertex f letexpr : all a. ([a] -> Ast_Expr -> [(Name, Info)]) -> Binding -> [(Name, Info)]
```
...
```
          ...
          <DocBlock title="_handleApps" kind="let">

```mc
let _handleApps id f prev g name2info app : all a. all l. all a1. all a2. a -> (Digraph (Name, Info) l -> a1 -> Map Name Info -> [a2] -> Ast_Expr -> [(a1, (Name, Info), a)]) -> a1 -> Digraph (Name, Info) l -> Map Name Info -> Ast_Expr -> [(a1, (Name, Info), a)]
```
