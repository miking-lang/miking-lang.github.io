...
<a href={"/docs/Stdlib/tuning/dependency-analysis.mc"} style={S.link}>dependency-analysis.mc</a>, <a href={"/docs/Stdlib/tuning/data-frame.mc"} style={S.link}>data-frame.mc</a>, <a href={"/docs/Stdlib/map.mc"} style={S.link}>map.mc</a>, <a href={"/docs/Stdlib/graph.mc"} style={S.link}>graph.mc</a>, <a href={"/docs/Stdlib/eqset.mc"} style={S.link}>eqset.mc</a>  
  
## Types  
  

          <DocBlock title="SearchSpaceSize" kind="type">

```mc
type SearchSpaceSize : { measuringPoints: Map Int Float, connectedComponents: [([Int], Float)], sizeTotal: Float, sizeReduced: Float }
```

<Description>{`NOTE: floats because of numerical overflows with ints`}</Description>
...
## Languages  
          ...
          <DocBlock title="SearchSpace" kind="lang" link="/docs/Stdlib/tuning/search-space.mc/lang-SearchSpace">
...
```
          ...
          <DocBlock title="Test" kind="lang" link="/docs/Stdlib/tuning/search-space.mc/lang-Test">
...
## Variables  
          ...
          <DocBlock title="searchSpaceProduct" kind="let">

```mc
let searchSpaceProduct seqs : all a. [[a]] -> [[a]]
```
...
<ToggleWrapper text="Code..">
```mc
let searchSpaceProduct : all a. [[a]] -> [[a]] = lam seqs.
...
```
          ...
          <DocBlock title="_productEq" kind="let">

```mc
let _productEq cmp p1 p2 : all a. (a -> a -> Int) -> [[a]] -> [[a]] -> Bool
```
...
```
          ...
          <DocBlock title="_searchSpaceTestEq" kind="let">

```mc
let _searchSpaceTestEq lhs rhs : Map Int Float -> [(Int, Float)] -> Bool
```
...
```
          ...
          <DocBlock title="searchSpaceSizeMeasuringPoint" kind="let">

```mc
let searchSpaceSizeMeasuringPoint points graph holeDomainSize : [Int] -> Graph Int Int -> [Float] -> Map Int Float
```
...
```
          ...
          <DocBlock title="searchSpaceSizeConnectedComponent" kind="let">

```mc
let searchSpaceSizeConnectedComponent pointSizes graph holeDomainSize : Map Int Float -> Graph Int Int -> [Float] -> [([Int], Float)]
```
