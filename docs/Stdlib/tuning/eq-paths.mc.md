...
<a href={"/docs/Stdlib/common.mc"} style={S.link}>common.mc</a>, <a href={"/docs/Stdlib/digraph.mc"} style={S.link}>digraph.mc</a>, <a href={"/docs/Stdlib/string.mc"} style={S.link}>string.mc</a>, <a href={"/docs/Stdlib/eqset.mc"} style={S.link}>eqset.mc</a>  
  
## Variables  
  

          <DocBlock title="eqPaths" kind="let">

```mc
let eqPaths g endNode depth startNodes : all v. all l. Digraph v l -> v -> Int -> [v] -> [[(v, v, l)]]
```

<Description>{`'eqPaths g endNode depth startNodes' computes representative paths in the  
...
'startNodes' and end in 'endNode'.`}</Description>
...
```
          ...
          <DocBlock title="eqPathsToLbls" kind="let">

```mc
let eqPathsToLbls paths : all v. all l. [[DigraphEdge v l]] -> [[l]]
```
