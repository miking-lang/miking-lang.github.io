...
<a href={"/docs/Stdlib/c/ast.mc"} style={S.link}>c/ast.mc</a>, <a href={"/docs/Stdlib/pmexpr/ast.mc"} style={S.link}>pmexpr/ast.mc</a>, <a href={"/docs/Stdlib/pmexpr/extract.mc"} style={S.link}>pmexpr/extract.mc</a>  
  
## Languages  
  

          <DocBlock title="PMExprCWrapper" kind="lang" link="/docs/Stdlib/pmexpr/wrapper.mc/lang-PMExprCWrapper">

```mc
lang PMExprCWrapper
```

<Description>{`Defines an extensible C wrapper generation language fragment. This fragment  
can be extended to implement the target\-specific parts of the wrapper, while  
allowing reuse of the parts that all targets will have in common.`}</Description>
...
## Variables  
          ...
          <DocBlock title="_bigarrayNumDimsKey" kind="let">

```mc
let _bigarrayNumDimsKey  : Name
```
...
```
          ...
          <DocBlock title="_bigarrayDimKey" kind="let">

```mc
let _bigarrayDimKey  : Name
```
...
```
          ...
          <DocBlock title="cWrapperNamesRef" kind="let">

```mc
let cWrapperNamesRef  : Ref (Option (Map String Name))
```

<Description>{`TODO\(larshum, 2022\-01\-19\): Figure out a way to make this set of names  
...
```
          ...
          <DocBlock title="_genCWrapperNames" kind="let">

```mc
let _genCWrapperNames _ : all a. a -> Map String Name
```
...
```
          ...
          <DocBlock title="getCWrapperNames" kind="let">

```mc
let getCWrapperNames _ : all a. a -> Map String Name
```
...
```
          ...
          <DocBlock title="updateCWrapperNames" kind="let">

```mc
let updateCWrapperNames newValue : Map String Name -> ()
```
...
```
          ...
          <DocBlock title="_getIdentExn" kind="let">

```mc
let _getIdentExn str : String -> Name
```

<Description>{`Maps a string to a \(hardcoded\) unique name of the C function.`}</Description>
...
```
          ...
          <DocBlock title="_getIdentOrInitNew" kind="let">

```mc
let _getIdentOrInitNew str : String -> Name
```
