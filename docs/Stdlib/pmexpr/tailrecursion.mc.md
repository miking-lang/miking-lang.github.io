...
<a href={"/docs/Stdlib/mexpr/eq.mc"} style={S.link}>mexpr/eq.mc</a>, <a href={"/docs/Stdlib/mexpr/symbolize.mc"} style={S.link}>mexpr/symbolize.mc</a>, <a href={"/docs/Stdlib/mexpr/type-check.mc"} style={S.link}>mexpr/type-check.mc</a>, <a href={"/docs/Stdlib/pmexpr/ast.mc"} style={S.link}>pmexpr/ast.mc</a>, <a href={"/docs/Stdlib/pmexpr/function-properties.mc"} style={S.link}>pmexpr/function-properties.mc</a>, <a href={"/docs/Stdlib/pmexpr/utils.mc"} style={S.link}>pmexpr/utils.mc</a>  
  
## Types  
  

          <DocBlock title="TailRecursiveEnv" kind="type">

```mc
type TailRecursiveEnv : { binop: Expr, ne: Expr, leftArgRecursion: Bool }
```
...
```
          ...
          <DocBlock title="TailPosInfo" kind="type">

```mc
type TailPosInfo : { binop: Expr, side: Option Side }
```

<Description>{`Finds the binary operator and the side at which the recursive call takes  
...
## Constructors  
          ...
          <DocBlock title="LeftSide" kind="con">
...
```
          ...
          <DocBlock title="RightSide" kind="con">
...
## Languages  
          ...
          <DocBlock title="PMExprTailRecursion" kind="lang" link="/docs/Stdlib/pmexpr/tailrecursion.mc/lang-PMExprTailRecursion">

```mc
lang PMExprTailRecursion
```
...
```
          ...
          <DocBlock title="TestLang" kind="lang" link="/docs/Stdlib/pmexpr/tailrecursion.mc/lang-TestLang">
...
## Variables  
          ...
          <DocBlock title="compatibleSide" kind="let">

```mc
let compatibleSide  : Option Side -> Option Side -> Option Side
```

<Description>{`Finds a compatible side which agrees with both given side values. The value  
None represents neither side \(the minimal element\), while Some \(BothSides \(\)\)  
represents either side \(the maximal element\).`}</Description>
...
```
          ...
          <DocBlock title="isRecursiveCall" kind="let">

```mc
let isRecursiveCall expr ident : Ast_Expr -> Name -> Bool
```

<Description>{`Determines whether a given expression is a recursive call to a function with  
the given identifier.`}</Description>
...
```
          ...
          <DocBlock title="tailPositionExpressionInfo" kind="let">

```mc
let tailPositionExpressionInfo ident expr : Name -> Ast_Expr -> Option TailPosInfo
```
