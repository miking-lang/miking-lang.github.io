...
<a href={"/docs/Stdlib/set.mc"} style={S.link}>set.mc</a>, <a href={"/docs/Stdlib/mexpr/anf.mc"} style={S.link}>mexpr/anf.mc</a>, <a href={"/docs/Stdlib/pmexpr/parallel-patterns.mc"} style={S.link}>pmexpr/parallel-patterns.mc</a>, <a href={"/docs/Stdlib/pmexpr/rules.mc"} style={S.link}>pmexpr/rules.mc</a>, <a href={"/docs/Stdlib/pmexpr/tailrecursion.mc"} style={S.link}>pmexpr/tailrecursion.mc</a>, <a href={"/docs/Stdlib/pmexpr/utils.mc"} style={S.link}>pmexpr/utils.mc</a>  
  
## Types  
  

          <DocBlock title="Argument" kind="type">
...
```
          ...
          <DocBlock title="PatternMatchState" kind="type">

```mc
type PatternMatchState : { active: [Int], patternDependencies: Map Int (Set Int), patternIndexToBoundVar: Map Int Name, atomicPatternMatches: Map Int Expr, nameMatches: Map Name Argument, variableDependencies: Map Name (Set VarPattern), pattern: Pattern }
```
...
## Languages  
          ...
          <DocBlock title="TestLang" kind="lang" link="/docs/Stdlib/pmexpr/pattern-match.mc/lang-TestLang">
...
## Variables  
          ...
          <DocBlock title="emptyPatternMatchState" kind="let">

```mc
let emptyPatternMatchState p : all a. all v. all v1. all v2. all v3. all v4. Pattern -> {active: [a], pattern: Pattern, nameMatches: Map Name v1, patternDependencies: Map Int v, atomicPatternMatches: Map Int v2, variableDependencies: Map Name v4, patternIndexToBoundVar: Map Int v3}
```
...
```
          ...
          <DocBlock title="isFinalState" kind="let">

```mc
let isFinalState state : PatternMatchState -> Bool
```
...
```
          ...
          <DocBlock title="getVariableIdentifier" kind="let">

```mc
let getVariableIdentifier varExpr : Ast_Expr -> Option Name
```

<Description>{`This function extracts the name of a TmVar and returns an option containing  
the name, assuming that the given expression is always a variable. If this  
assumption does not hold, it returns None.`}</Description>
...
```
          ...
          <DocBlock title="findVariableDependencies" kind="let">

```mc
let findVariableDependencies state expr : PatternMatchState -> Ast_Expr -> Set VarPattern
```
...
```
          ...
          <DocBlock title="updateVariableDependencies" kind="let">

```mc
let updateVariableDependencies state ident body optPatIdx : PatternMatchState -> Name -> Ast_Expr -> Option Int -> PatternMatchState
```
...
```
          ...
          <DocBlock title="applyPattern" kind="let">

```mc
let applyPattern state boundVar expr patIndex : PatternMatchState -> Name -> Ast_Expr -> Int -> PatternMatchState
```
...
```
          ...
          <DocBlock title="matchVariablePattern" kind="let">

```mc
let matchVariablePattern expr state varPat : Ast_Expr -> PatternMatchState -> VarPattern -> Option PatternMatchState
```
...
```
          ...
          <DocBlock title="matchAtomicPattern" kind="let">

```mc
let matchAtomicPattern bindingIdent params expr state patIdx : Name -> [(Name, Ast_Type, Info)] -> Ast_Expr -> PatternMatchState -> Int -> Option PatternMatchState
```
...
```
          ...
          <DocBlock title="matchAtomicPatterns" kind="let">

```mc
let matchAtomicPatterns bindingIdent params expr state : Name -> [(Name, Ast_Type, Info)] -> Ast_Expr -> PatternMatchState -> Option PatternMatchState
```
...
```
          ...
          <DocBlock title="constructLookup" kind="let">

```mc
let constructLookup state : PatternMatchState -> Map VarPattern (Name, Ast_Expr)
```
...
```
          ...
          <DocBlock title="matchPattern" kind="let">

```mc
let matchPattern binding pattern : LetDeclAst_DeclLetRecord -> Pattern -> Option (Map VarPattern (Name, Ast_Expr))
```
