...
Create the Opam 5.0 beta switch:

```
opam update
opam switch create ocaml5-beta ocaml-variants.5.0.0~beta2+options
eval $(opam env)
```
...
If using Mac with Apple Silicon, you might have to include the `--platform=linux/amd64` parameter when running the image.
</TabItem>
...
# Mount Directory that we are running the script from
...
Example MExpr program that you can use to verify your installation. Save this
as `fib.mc` and compile it with `mi compile fib.mc`. Try running it as
`./fib 50`.
